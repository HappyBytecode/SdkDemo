...
//					drawables[3]);
    ...
    public void setDrawalbes(int left, int top, int right, int bottom) {
        int[] res = {left, top, right, bottom};
        Drawable[] drawables = new Drawable[res.length];
        for (int i = 0; i < res.length; i++) {
            drawables[i] = res[i] == 0 ? null : getContext().getResources().getDrawable(res[i]);
        }
        setDrawables(drawables);
    }
