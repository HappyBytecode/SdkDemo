 ...
 * 标题控件
 */
public class HeadViewWhite extends FrameLayout {
    ...
    public HeadViewWhite(Context context) {
        this(context, null);
    }

    public HeadViewWhite(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HeadViewWhite(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        LayoutInflater.from(context).inflate(R.layout.hxyc_anda_head_view_white, this);
        ...
        //标题颜色，默认显示"白色"
        int headColor = ta.getColor(R.styleable.HeadView_title_color, context.getResources().getColor(R.color.head_view_center_black));
        ...
        if (ta.hasValue(R.styleable.HeadView_left_image_tint)) {
            ColorStateList color = ta.getColorStateList(R.styleable.HeadView_left_image_tint);
            mLeft.setImageTintList(color);
        }
        ...
        // 控制下方线的显示
        boolean isDivideLineVis = ta.getBoolean(R.styleable.HeadView_divide_line_visible, false);
